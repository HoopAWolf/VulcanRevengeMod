    ...
    public void tick()
    {
        ...
        if (!this.world.isRemote)
        {
            if (this.inGround && this.timeInGround != 0)
            {
                ...
                SpawnParticleMessage spawnParticleMessage = new SpawnParticleMessage(new Vec3d(this.getPosX(), this.getPosY(), this.getPosZ()), new Vec3d(0.0D, 0.0D, 0.0D), 3, 5, 0.0F);
                VRMPacketHandler.packetHandler.sendToDimension(this.dimension, spawnParticleMessage);
            }
            ...
            SpawnParticleMessage spawnParticleMessage = new SpawnParticleMessage(new Vec3d(this.getPosX() + d3, this.getPosY() + d4, this.getPosZ() + d0), new Vec3d(0.0D, 0.0D, 0.0D), 3, 5, 0.0F);
            VRMPacketHandler.packetHandler.sendToDimension(this.dimension, spawnParticleMessage);
        }
    }
